...
class GroupManager(
    ...
    fun removeGroup(groupData: GroupData<*>) {
        if (groupDataList.contains(groupData)) {
            ...
            adapter.notifyItemRangeRemoved(groupData.positionInGroup, groupData.getCount())
        }
    }
    ...
    fun getItemViewType(adapterPosition: Int): Int {
        ...
        return data?.getItemViewType(adapterPosition - data.positionInGroup)
    ...
    fun getItemDataAtAdapterPosition(adapterPosition: Int): Any {
        val data: GroupData<*>? = findGroupDataByAdapterPosition(adapterPosition)
        return data?.getDataInGroup(adapterPosition - data.positionInGroup)
            ?: throw IllegalArgumentException("Can not find data for position: $adapterPosition")
    }
    ...
    fun notifyChanged(group: GroupData<*>) {
        adapter.notifyItemRangeChanged(group.positionInGroup, group.getCount())
    }

    fun notifyChanged(group: GroupData<*>, payload: Any?) {
        adapter.notifyItemRangeChanged(group.positionInGroup, group.getCount(), payload)
    }
    ...
    fun notifyNewGroupAdded(group: GroupData<*>) {
        ...
        adapter.notifyItemRangeInserted(group.positionInGroup, group.getCount())
    }
    ...
    fun findGroupDataByAdapterPosition(adapterPosition: Int): GroupData<*>? {
        groupDataList.forEach {
            if (it.isAttached()) {
                if (it.positionInGroup <= adapterPosition
                    && adapterPosition < it.positionInGroup + it.getCount()
                ) {
                    return it
                }
            }
        ...
        return when {
            index > 0 -> {
                for (i in index - 1 downTo 0) {
                    val prev: GroupData<*> = groupDataList.get(i)
                    if (prev.isAttached()) {
                        return prev.positionInGroup + prev.getCount()
                    }
                }
                0
            }
